...
MolBlender provides a versatile interface to [Uni-Mol](https://github.com/dptech-corp/Uni-Mol), a powerful deep learning framework for 3D molecular modeling. It leverages the `unimol_tools` library to generate distinct types of molecular representations:

* **Molecule-level (CLS) embeddings**: A single vector representing the entire molecule, obtained via {py:class}`~molblender.representations.spatial.unimol.UniMolCLSFeaturizer`.
* **Atom-level representations**: An embedding vector for each atom in the molecule, obtained via {py:class}`~molblender.representations.spatial.unimol.UniMolAtomicReprsFeaturizer`.
* **Atomic 3D coordinates**: The spatial coordinates of each atom, typically generated by `unimol_tools`' internal conformer generation, obtained via {py:class}`~molblender.representations.spatial.unimol.UniMolAtomicCoordsFeaturizer`.

These featurizers are designed to be 3D-aware. When provided with SMILES strings, `unimol_tools` will internally generate 3D conformers before featurization. Each specific featurizer variant (CLS, Coords, AtomicReprs) returns a single NumPy array, adhering to the standard MolBlender featurizer interface.

---
## Dependencies

To utilize UniMol featurizers within MolBlender, you'll need to install their core dependencies. You can install them together:
...
Models are typically downloaded automatically by `unimol_tools` (via `huggingface_hub`) to a local cache. For managing cache locations effectively with MolBlender, please refer to our guide on {doc}`../../basic/config`.

---
## Key Concepts

-   **Specialized Featurizers**: MolBlender offers distinct featurizer classes for each primary type of UniMol output:
    -   {py:class}`~molblender.representations.spatial.unimol.UniMolCLSFeaturizer`: For molecule-level embeddings. Returns a 1D NumPy array.
    -   {py:class}`~molblender.representations.spatial.unimol.UniMolAtomicCoordsFeaturizer`: For 3D atomic coordinates. Returns a 2D NumPy array of shape `(N_atoms, 3)`.
    -   {py:class}`~molblender.representations.spatial.unimol.UniMolAtomicReprsFeaturizer`: For atom-level embeddings. Returns a 2D NumPy array of shape `(N_atoms, D_embedding)`.
...
Access UniMol featurizers using their registered names with {py:func}`molblender.representations.get_featurizer`. Each featurizer variant is registered with a descriptive key, detailed in the "Available Pre-registered Configurations" section.
...
{py:class}`~molblender.representations.spatial.unimol.UniMolCLSFeaturizer`,
{py:class}`~molblender.representations.spatial.unimol.UniMolAtomicCoordsFeaturizer`, or
{py:class}`~molblender.representations.spatial.unimol.UniMolAtomicReprsFeaturizer`.
...
-   `model_config_key` refers to a key in the `UNIMOL_BASE_CONFIGS` dictionary within `molblender.representations.spatial.unimol`. This determines the base UniMol model settings (like `unimol_tools_model_name`, `unimol_tools_model_size`, and the default `remove_hs`).
...
Each UniMol featurizer variant in MolBlender returns a single NumPy array (`np.float32`) for a successfully processed molecule, or `None` if featurization fails. The specifics are detailed below:
...
MolBlender uses a set of base Uni-Mol model configurations defined in {py:data}`molblender.representations.spatial.unimol.UNIMOL_BASE_CONFIGS`. For each entry in this dictionary, three featurizer variants are automatically registered: one for CLS representations, one for atomic coordinates, and one for atomic representations.


MolBlender registers specific featurizer variants based on the `UNIMOL_BASE_CONFIGS` defined in {py:mod}`molblender.representations.spatial.unimol`. The registered names follow the pattern: `UniMol-[Type]-[ToolModelName]-[ToolModelSizeIfAny]-[HsHandling]`.
...
The following table details the base configurations defined in MolBlender. Each of these generates the three featurizer variants mentioned above.

| MolBlender Base Config Key   | `unimol_tools` `model_name` | `unimol_tools` `model_size` | Default `remove_hs` | Assumed Output Dim. (D) | `data_type` |
| :---------------------------- | :-------------------------- | :-------------------------- | :-------------------- | :------------------------ | :---------- |
...
-   {py:class}`~molblender.representations.spatial.unimol.UniMolCLSFeaturizer`
-   {py:class}`~molblender.representations.spatial.unimol.UniMolAtomicCoordsFeaturizer`
-   {py:class}`~molblender.representations.spatial.unimol.UniMolAtomicReprsFeaturizer`
-   (Internal base class for UniMol featurizers) {py:class}`molblender.representations.spatial.unimol.BaseUniMolFeaturizer`
